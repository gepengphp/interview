...
- 前缀索引  
    ...
    > 
    > -- 可以使用分组查询，查看不同长度的选择性，保证选择性的同时，使用最小的空间
    > select count(*) as cnt,left(name, 5) as pref from users group by pref order by cnt desc limit 10;
    +-----+--------------+
    | cnt | name         |
    +-----+--------------+
    |   2 | china        |
    +-----+--------------+
    > select count(*) as cnt,left(name, 6) as pref from users group by pref order by cnt desc limit 10;
    +-----+--------------+
    | cnt | name         |
    +-----+--------------+
    |   1 | chinad       |
