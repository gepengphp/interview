...
> - 所以 `Innodb` 的每张表对应两个文件：`frm` 结构文件、`idb` 数据文件。`Myisam` 每张表对应三个文件：`frm` 结构文件、`MYD` 数据文件、`MYI` 索引文件。  
> - 所以 `idb` 文件种的数据就是一个聚簇索引（主键作为树结点的 `b+tree`）的结构存储。  
> - 所以每张表只有一个聚簇索引。
> - 所以 `InnoDB` 引擎无论是否创建索引，都会自动设置一个索引，就是为了构造聚簇索引的结构保存数据。 
