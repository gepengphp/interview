...
以可重复读级别为例。
当一条查询 sql 执行的时候，获取当前查询开始时，所有**已开始 - 未提交**状态的所有**事务 ID** 和**最大事务 ID**，与当前结果行的事务 ID 进行比较。如果结果行的事务 ID 存在于查询时记录的事务 ID，说明当前结果行是幻读行，需要查询旧版本。再根据当前结果的 `roll_pointer` 查询历史版本，直到历史版本的事务 ID 没有出现在当前查询记录的事务 ID 中，说明找到了这一行应该被读取的状态。